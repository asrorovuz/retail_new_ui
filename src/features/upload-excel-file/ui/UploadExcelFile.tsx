...
const optionSelect = [
    ...
    { label: "Оптовая цена", value: "bulkPrice" },
    { label: "Тип товара (Белый/Чёрный)", value: "isLegal" },
...
const ProductHeader = ({
    ...
    const handleIncDec = (type: "+" | "-") => {
        setInitialState((prev: any) => ({
            ...
            rowsCount: Math.max(
                0,
                Number(prev.rowsCount) + (type === "+" ? 1 : -1),
            ),
        }));
    };
                ...
                >
                    <Switcher
                        ...
                        onChange={(checked: boolean) =>
                            upDateInitialState("edit", checked)
                        }
                    />
            ...
            <Table cellBorder={true} className="w-full table-fixed">
                ...
                <TBody>
                    {paginatedData?.map((elem: string[], index: number) => {
                        ...
                        const highlight =
                            Number(initialState?.rowsCount) >= globalIndex;
                        return (
                            <Tr
                                key={`row-${globalIndex}`}
