...
const EditFiscalDevice = ({ getCashRegisterData }: any) => {
    ...
    const { mutate: updateMutateArca, isPending: arcaLoading } =
        useUpdateArca();
    const { mutate: updateMutateSimurg, isPending: simurgLoading } =
        useUpdateSimurg();
    const { mutate: updateMutateHippoPos, isPending: hippoposLoading } =
        useUpdateHippoPos();
    const { mutate: updateMutateHippoPos4, isPending: hippoposLoading4 } =
        useUpdateHippoPos4();
    const { mutate: updateMutateEPos, isPending: eposLoading } =
        useUpdateEPos();
        ...
        methods.reset({
            ...
            ...(Number(getCashRegisterData?.type) ===
                ...
                hippopos: {
                    ...
                    companyAddress:
                        getCashRegisterData?.info?.company_address || "",
                    companyInn: getCashRegisterData?.info?.company_inn || "",
                    printerSize:
                        String(getCashRegisterData?.info?.printer_size) || 0,
                },
            }),
            ...
            ...(Number(getCashRegisterData?.type) ===
                ...
                simurg: {
                    ...
                    cashierPassword:
                        String(getCashRegisterData?.info?.cashier_password) ||
                        0,
                },
            }),
            ...
            ...(Number(getCashRegisterData?.type) ===
                ...
                epos: {
                    ...
                    companyInn: getCashRegisterData?.info?.company_inn || "",
                    printerSize:
                        String(getCashRegisterData?.info?.printer_size) || "",
                },
            }),
        });
    };
    ...
    const onSubmit = (item: any) => {
        ...
        if (Number(item.type) === CashRegisterProviderTypeHippoPos4) {
            updateMutateHippoPos4(
                {
                    id: item.id,
                    payload: {
                        name: item.name,
                        is_enabled: item.isEnabled,
                        company_name: item.hippopos4?.companyName,
                        company_address: item.hippopos4?.companyAddress,
                        company_inn: item.hippopos4?.companyInn,
                        printer_size: Number(item.hippopos4?.printerSize),
                    },
                },
                {
                    onSuccess() {
    ...
    let loading =
        simurgLoading || arcaLoading || eposLoading || hippoposLoading || hippoposLoading4;
