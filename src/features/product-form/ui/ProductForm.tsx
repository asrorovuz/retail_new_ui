...
const ProductForm: FC<ProductFormType> = ({
            ...
            ...(type !== "edit"
                ? {
                      purchase_price: {
                          ...
                          currency_code:
                              Number(values?.purchase_price?.currency?.code) ??
                              0,
                      },
                  }
                : {}),
                ...
                { productId, data },
                {
                    onSuccess(res) {
                        ...
                        if (values?.purchase_price) {
                            const payload = {
                                amount:
                                    Number(values?.purchase_price?.amount) ?? 0,
                                currency_code:
                                    Number(
                                        values?.purchase_price?.currency?.code,
                                    ) ?? 0,
                                warehouse_id: wareHouseId,
                                product_id: res?.id,
                            };
                ...
                onSuccess(res) {
                    ...
                    if (pageType !== "products") {
                        ...
                        const newItem = {
                            ...
                            quantity: quantity + 1,
                            totalAmount:
                                (quantity + 1) * (packagePrice?.amount ?? 0),
    ...
    useEffect(() => {
        ...
        const hasBarcode = list?.some(
            (item: any) => item?.value === catalogItem?.barcode,
        );
        if (!hasBarcode && catalogItem?.barcode) {
            setValue(
                "barcodes",
                [...list, { value: catalogItem.barcode, count: 1 }],
                {
                    shouldDirty: true,
                },
            );
